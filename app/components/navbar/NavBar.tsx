...
export default function WithSubnavigation() {
  ...
  return (
    <div>
        <div className="bg-brand-blue shadow-xl text-ftnal-white min-h-[4rem] py-2 px-4 border-b border-gray-200 flex items-center md:justify-between">
      ...
      {NAV_ITEMS.map((navItem) => (
        <Box key={navItem.label}>
          <Popover trigger={'hover'} placement={'bottom-start'}>
            ...
            {navItem.children && (
              <PopoverContent
                className="border-0 shadow-xl bg-popover bg-ftnal-white text-ftnal-black p-4 rounded-xl min-w-sm">
                <Stack>
...
const DesktopSubNav = ({ label, href, subLabel }: NavItem) => {
  return (
    <a
      role="group" className="group block p-2 rounded-md transition-all hover:bg-ftnal-offwhite"
      href={href}>
      <Stack direction={'row'} align={'center'}>
        ...
        <Flex
          ...
          align={'center'}
          flex={1}>
          <Icon className="w-5 h-5" as={ChevronRightIcon} />
        </Flex>
      </Stack>
    </a>
  )
...
const MobileNavItem = ({ label, children, href }: NavItem) => {
  ...
  return (
    <Stack className="bg-brand-yellow" spacing={4} onClick={children && onToggle}>
      <Box
      className="bg-brand-yellow py-2 flex flex-row items-center"
        ...
        <p className="font-bold text-ftnal-white">
          {label}
        </p>
        {children && (
          <Icon
      ...
      <Collapse in={isOpen} animateOpacity style={{ marginTop: '0!important' }}>
        <Stack
        className="mt-2 pl-4 align-start border-l-2 border-l-white"
          >
          {children &&
            children.map((child) => (
              <Box as="a" key={child.label} href={child.href} className="py-2 text-ftnal-white text-dmsans text-body-l">
                {child.label}
              </Box>
            ))}
        </Stack>
